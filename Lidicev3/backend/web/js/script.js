...
$(document).ready(function()
{
	...
	$(document).on('keydown', '.arancel',function(event)
	{
		...
		if (event.which == 13)
		{
			...
			parentTr.addClass("arn_"+hex_md5(ArancelRow.id+ArancelRow.nombre));
			parentTr.attr("data-arn","arn_"+hex_md5(ArancelRow.id+ArancelRow.nombre));
...
$(document).ready(function() {
	...
	// on blur description
$(document).on("blur",".descripciyon",function(){
	var desparentTr = $(this).closest('tr');
	var arn_data = desparentTr.attr("data-arn");
	desparentTr.removeClass(arn_data);
	//desparentTr.find(".arancelid");
	//this.val();
	var newmd5str = "arn_"+hex_md5(desparentTr.find(".arancelid").val()+$(this).val());
    desparentTr.attr("data-arn",newmd5str);
	desparentTr.addClass(newmd5str);
});

	// on click consolidate
$(document).on("click","#consolidate",function(){
	findDuplicate();
	$("#calculatesheet").click();
});

$(document).on("click","#customPeso",function(){
	   if($(this).is(':checked')){
	        enableCustomPeso();
	    } else {
	        disableCustomPeso();
      ...
      $(".dynamic_row_table .calc_tr").each(function () {
      		...
      		if (current_same > 1) {
        		...
        		//alert(nextTextboxCls);
      			...
      			sumsame(current_cls+" ."+nextTextboxCls);

      			if ($("#customPeso").is(':checked')) {
					sumsame(current_cls+" .peso");
				}
...
function cif_total() {
//CIF total will be sum of FOB + Insurance + Freigth.
		var cost = checksetval( $("#cost").text() );
		var insurance = checksetval( $("#insurance").text() );
		var freight = checksetval( $("#freight").text() );

		var cif_total = cost + insurance + freight;

		$("#ciftotal").html( cif_total.toFixed(2) );

		accurateCIF();
}

function data_to_process() {
	var DataToSubmit = [];
		$(".dynamic_row_table tr.calc_tr").each(function(){
			...
			if ($(this).find('.arancelid').val() > 0) {
				...
				if ($("#customPeso").is(':checked')) {
					var item_peso = $(this).find('.peso').val();
				}
...
// trying to overwrite builtin function
function mytoFixed(num, fixed) {
    var re = new RegExp('^-?\\d+(?:\.\\d{0,' + (fixed || -1) + '})?');
    return num.toString().match(re)[0];
}

function accurateCIF() {

	var acc_allowed = ["cif_pueto", "fob", "cfr", "fobg"];
    var acc_ind = acc_allowed.indexOf(g_typeo);

	if (acc_ind == -1 ) {return;}

	var acc_ciftotal = $("#ciftotal").html();// bottom
	var acc_cif_total =  $("#cif-total").val(); //top always accurate????

	if (acc_ciftotal != acc_cif_total) {
		//set cif total to accurate
		//alert(acc_ciftotal);
		//alert(acc_cif_total);
		if (g_typeo == 'fob') {
			$("#cif-total").val(acc_ciftotal); //set top
		} else {
			$("#ciftotal").html(acc_cif_total); //set bottom
			var acc_diff = acc_cif_total - acc_ciftotal;
			var acc_fob = parseFloat( $("#fob-total").val() ) + parseFloat (acc_diff.toFixed(2) );
			//update fob and cost
			$("#fob-total").val(acc_fob.toFixed(2));
			$("#cost").text(acc_fob.toFixed(2));
		}
	}
}

function enableCustomPeso() {
	//event.preventDefault();
	    $(".peso").each(function(){
	    	$(this).attr("disabled",false);
	    });

	    $("#poso-total").attr("disabled",true);
}

function disableCustomPeso() {
	//event.preventDefault();
	    $(".peso").each(function(){
	    	$(this).attr("disabled",true);
	    });

	    $("#poso-total").attr("disabled",false);
}
