...
class LiquidationController extends Controller
{
    ...
    /*
    * call Arancel by ajax
    */
    public function actionArancelbyajax()
    {
...
/*
		...
		//$RS = self::find()->limit(100)->orderBy('codigo')->all();
                $RS = Arancel::find()->orderBy('codigo')->all();
...
echo "[";
		...
		//$RS = Arancel::find()->limit(50)->offset(520)->orderBy('codigo')->all();
                $RS = Arancel::find()->orderBy('codigo')->all();
...
//echo '{"id":"'.$row->id.'","code":"'.$row->codigo.'","nombre":"'.stripslashes ($row->descripcion).'","dia":"'.$row->dia.'","itbm":"'.$row->itbm.'","isc":"'.$row->isc.'"},'."\n";

if ($total_rs == $ind) {
echo '{"id":"'.$row->id.'","code":"'.$row->codigo.'","nombre":"'.str_replace('"', '', $row->descripcion).'","dia":"'.$row->dia.'","itbm":"'.$row->itbm.'","isc":"'.$row->isc.'"}';
} else {
echo '{"id":"'.$row->id.'","code":"'.$row->codigo.'","nombre":"'.str_replace('"', '', $row->descripcion).'","dia":"'.$row->dia.'","itbm":"'.$row->itbm.'","isc":"'.$row->isc.'"},';
}
...
//echo '{"id":2,"code":"0101.29.00.00","nombre":"Los dems"}';

//echo '[{"id":"2","code":"0101.29.00.00","nombre":"Los demás","dia":"10","itbm":"0","isc":"0"},{"id":"3","code":"0101.30.00.00","nombre":"Asnos","dia":"10","itbm":"0","isc":"0"},{"id":"4","code":"0101.90.00.00","nombre":"Los demás","dia":"10","itbm":"0","isc":"0"}]';
